...
public class NewReservationController {
    ...
    public void initialize() {
        ...
        try {
            ...
            AlertHelper.DisplayErrorAlert("Error occured retrieving departments", "");
            e.printStackTrace();
        }
        ...
        if ((roomCapacityTextField.getText().length() == 0 || roomDeptChoiceBox.getValue() == null)
            ...
            try {
                ...
                if (startDate.before(getEventManagerController().getSelectedEvent().getStartDateTime()) || endDate.after(getEventManagerController().getSelectedEvent().getEndDateTime())) {
                    AlertHelper.DisplayErrorAlert("Error on input parameters", "Reservation must be within the event timespan");
                    return;
                }
                ...
                if (roomManagementService.reserveRoom(eventManagerController.getSelectedEvent().getEventID(), roomProperties, roomDeptChoiceBox.getValue(), startDate, endDate) != null) {
                    ...
                    AlertHelper.DisplayErrorAlert("Room was not reserved", "No room with given properties is available");
                }
            } catch (AuthRequiredException e) {
                AlertHelper.DisplayErrorAlert("User authentication failed", "");
                e.printStackTrace();
            } catch (RoomServiceException | ReservationServiceException e) {
                AlertHelper.DisplayErrorAlert("Reservation creation failed", "");
                e.printStackTrace();
            }
        }
