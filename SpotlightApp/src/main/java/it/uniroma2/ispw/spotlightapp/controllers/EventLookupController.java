...
public class EventLookupController {
    ...
    private void searchByUser() {
        try {
            ...
            e.printStackTrace();
            it.uniroma2.ispw.spotlightapp.controllers.helpers.AlertHelper.DisplayErrorAlert("Error occured retrieving events", "");
        }
    }

    private void searchByNameAndDate() {
        if (eventDatePicker.getValue() == null) {
            it.uniroma2.ispw.spotlightapp.controllers.helpers.AlertHelper.DisplayErrorAlert("Error on input parameters", "Insert a valid date");
            return;
        }
        ...
        if (eventDate.getTime() < CalendarHelper.getToday().getTime()) {
            it.uniroma2.ispw.spotlightapp.controllers.helpers.AlertHelper.DisplayErrorAlert("Error on input parameters", "Date must be valid, present or future");
        }

        // check on text field
        if (searchTextField.getText().length() == 0){
            it.uniroma2.ispw.spotlightapp.controllers.helpers.AlertHelper.DisplayErrorAlert("Error on input parameters", "Event search text must be not empty");
        }
